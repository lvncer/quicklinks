...
## M4.3: OGP 取得の保存時集約（Read API は DB 参照のみ）

- **目的**:
  - 一覧表示などの Read 系 API は **DB 参照のみ**にして、外部サイト取得に依存しない安定動作に寄せる。
  - OGP 取得（＋フォールバック）は **リンク保存時に集約**し、保存済みデータで表示を完結させる。
- **やること**
  - API（保存時）
    - `POST /api/links` で OGP 取得を実施し、`og_image` / `description` / `title` をできる限り埋めて保存する。
    - 取得失敗時のフォールバック（例: `r.jina.ai` 等）や「Just a moment...」等の汚染防止を保存時ロジックに集約する。
  - API（読取時）
    - `GET /api/og` は **外部サイトを取りに行かず**、DB の保存済みメタデータを返すだけにする（必要なら将来削除も検討）。
    - `GET /api/links` は DB の `og_image` / `description` をそのまま返し、フロントはそれを表示する。
  - Web
    - リスト表示時の `/api/og` 依存（リアルタイム取得）を減らし、DB 値表示を基本にする（必要なときだけ段階的に見直す）。
- **メモ**
  - openai.com など外部取得が不安定なサイトは「保存時に取れた分だけ保存する」方針で割り切り、Read 時に外部依存を持ち込まない。
