...
- DB スキーマは **baseline（初期状態）を起点** にしつつ、以降は Ent schema + Atlas versioned migrations で管理している。
  - ent は「既存テーブルに対する ORM クライアント」として動きつつ、スキーマ変更も同一のソース（Ent schema）に寄せていく。
...
- 旧 `infra/migrations` は廃止し、Atlas versioned migrations（`api/ent/migrate/migrations/`）に統一する。

### 8-3. 追記（M4.0〜）：Ent/Atlas の versioned migrations を導入した

M4.0 から、`links` テーブルに関する DB スキーマ変更は **Ent schema を唯一のソース** としつつ、SQL は **Atlas の versioned migrations** で管理する形に寄せています。

- 設定ファイル: `api/atlas.hcl`
- マイグレーションディレクトリ: `api/ent/migrate/migrations/`
- baseline（既存DB向けの初期バージョン）:
  - `api/ent/migrate/migrations/20251212121929_baseline.sql`
  - 既存 Supabase DB はこの SQL を実行せず、**baseline として「適用済み」扱い**にします（下の手順参照）

以降は、基本的に次の流れです:

1. **Ent schema を変更**（例: `api/ent/schema/link.go`）
2. **コード生成**: `go generate ./ent`
3. **差分 SQL（マイグレーション）生成**: `atlas migrate diff <name> --env dev`
4. **Supabase に適用（手動）**: `atlas migrate apply --env supabase`

#### baseline 運用（既存 Supabase DB を起点にする）

既存DB（すでに `links` が存在する Supabase）に対しては、次を実行して **baseline 版まで適用済み**として登録します:

```bash
cd api
export DATABASE_URL="..." # Supabase の DATABASE_URL（session pooler 推奨）
atlas migrate apply --env supabase --baseline 20251212121929
```

その後、以降のマイグレーションは通常通り `atlas migrate apply --env supabase` で適用します。

#### よく使うコマンド（api ディレクトリで実行）

```bash
# マイグレーション生成（Ent schema -> SQL）
atlas migrate diff <name> --env dev

# Supabase へ適用（dry-run）
atlas migrate apply --dry-run --env supabase
...
1. **スキーマを変えたいとき**
   ...
   - `atlas migrate diff <name> --env dev` で差分マイグレーションを生成。
   - `atlas migrate apply --env supabase` で Supabase に手動適用（既存DBは baseline を先に設定）。
